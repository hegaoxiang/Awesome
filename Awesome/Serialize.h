...
{
public:
	static std::string Serialize(const entt::registry& currentReg)
	{
		
		NJSONOutputArchive json_archive;
		entt::basic_snapshot snapshot(currentReg);
		snapshot.entities(json_archive)
			.component<
			Component::TRS, 
			Component::Mesh, 
			Component::Material>(json_archive);
		json_archive.Close();
		return json_archive.AsString();
	}
	static void UnSerialize(entt::registry& reg,const std::string& data) throw(const std::exception&)
	{
